...
void writeInfo() {
  ...
  if (showBestEachGen) { //if showing the best for each gen then write the applicable info
    ...
    textSize(20);
    int x = 580;
    text("Расстояние до следующего препятствия", x, 18+44.44444);
    text("Высота препятствия", x, 18+2*44.44444);
    text("Ширина препятствия", x, 18+3*44.44444);
    text("Высота птицы", x, 18+4*44.44444);
    text("Скорость", x, 18+5*44.44444);
    text("Позиция игрокак (Y)", x, 18+6*44.44444);
    text("Расстояние между препятствиями", x, 18+7*44.44444);
    text("Смещение", x, 18+8*44.44444);

    textAlign(LEFT);
    text("Малый прыжок", 1220, 118);
